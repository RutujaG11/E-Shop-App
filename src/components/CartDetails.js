...
const CartDetails = () => {
  ...
  const calculateTotalPrice = () => {
    ...
    data.forEach((product) => {
      totalPrice += product.price * (quantities[product.id] || 1); // Adjust price based on quantity
    });
    ...
    <div className="cart">
      <div className="container">
            ...
            <div className="cart-item" key={index}>
              <div className="row">
                ...
                <div className="col-md-5 center-item">
                  <span onClick={() => decrementQuantity(product.id)} className="material-symbols-outlined">remove</span>
                  <label className="quantity" htmlFor={`quantity-${product.id}`}>Qty:- {quantities[product.id] || 1}</label>
                  <span onClick={() => incrementQuantity(product.id)} className="material-symbols-outlined">add</span>
                  <h5>Price: ${(product.price * (quantities[product.id] || 1)).toFixed(2)}</h5>
                  <span
        ...
        <div className="col-md-12 pt-4 pb-4">
          ...
          <Link to="/checkout">
            <button onClick={emptycart} className="btn btn-success check-out">
              Checkout
            </button>
          </Link>
        </div>
